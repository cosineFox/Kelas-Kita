  ...
  return (
    <main className="moderation-login">
      <form onSubmit={submit}>
        ...
        <p>Reviewers post without public accounts. Use this queue to handle appeals, reports and overrides.</p>
        ...
        <a href="/"><ArrowLeft /> Return to KelasKita</a>
      </form>
    </main>
  );
}
    ...
    <main className="moderation-console">
      <header className="moderation-topbar">
        ...
        <nav>
          ...
          <button onClick={async () => { await adminLogout(); setAuthenticated(false); }}>End session</button>
        </nav>
      </header>
      ...
      <section className="moderation-status">
        ...
        <button disabled={busy} onClick={() => load(true)}>Run live AI test</button>
      </section>
      ...
      <div className="moderation-workspace">
        ...
        <article className="case-detail">
          {!selected ? <div className="queue-empty"><ShieldAlert /><strong>Select a case</strong></div> : <>
            ...
            <section className="case-copy"><h3>Content under review</h3><blockquote>{selected.body}</blockquote></section>
            {selected.details && <section className="case-copy"><h3>Case context</h3><p>{selected.details}</p></section>}
            ...
            <section className="human-decision">
              ...
              {["retry", "dead"].includes(selected.jobState) && <button className="retry-button" disabled={busy} onClick={retry}><RefreshCw /> Retry Qwen check</button>}
            </section>
          </>}
        </article>
      </div>
